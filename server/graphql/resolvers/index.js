...
module.exports = {
  ...
  User: {
    reputation: (parent) => {
      const questionRep = parent.questions.reduce((sum, q) => sum + (q.rep * 2.5), 0);
      const answerRep = parent.answers.reduce((sum, a) => sum + (a.rep * 5), 0);
      if(questionRep + answerRep > 0)  
      {
        return Math.round((0 + questionRep/1.5 + answerRep/2)/5) ;
      }else{
        return Math.round((0 + questionRep + answerRep)/3);
      }
